...
WIDGET_LABEL="Gridtracker"

# Define the application name (lowercase, no spaces) and start command (use full path /home/pi/... and not ~/...)
APP_NAME="gridtracker"  # ** APP_NAME MUST match the script name (after the start_  prefix) for proper app-status API tracking **
APP_COMMAND="gridtracker2"
